...
// createSRATable populates the denormalized sra table by joining all exported tables
func (e *Exporter) createSRATable() error {
	...
	SELECT
		...
		CAST(NULL AS REAL) as spots,
		CAST(NULL AS REAL) as bases,
		r.run_center,
		r.experiment_name,
		r.run_url_link,
		r.run_entrez_link,
	...
	LEFT JOIN sample sa ON e.sample_accession = sa.sample_accession
	JOIN study st ON e.study_accession = st.study_accession
	LEFT JOIN submission sub ON e.submission_accession = sub.submission_accession`
	...
	// Update spots and bases from source database
...
// updateSRAMetrics updates spots and bases in the sra table from source data
func (e *Exporter) updateSRAMetrics() error {
	rows, err := e.sourceDB.DB.Query(`
		SELECT run_accession, total_spots, total_bases
		FROM runs
		WHERE total_spots > 0 OR total_bases > 0
	`)
	if err != nil {
		return err
	}
	...
	stmt, err := tx.Prepare(`UPDATE sra SET spots = ?, bases = ? WHERE run_accession = ?`)
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for rows.Next() {
		...
		var spots, bases int64
		if err := rows.Scan(&accession, &spots, &bases); err != nil {
			return err
		}
		if _, err := stmt.Exec(float64(spots), float64(bases), accession); err != nil {
			return err
		}
	}
