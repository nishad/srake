...
func NewIndexBuilder(cfg *config.Config, db *database.DB, backend search.SearchBackend, options BuildOptions) (*IndexBuilder, error) {
	...
	// use vectors — either Bleve KNN ("vectors"/FAISS) or the zvec index ("zvec").
	// Otherwise generating embeddings is pure overhead, so skip it and explain.
	if options.WithEmbeddings && !search.VectorSearchSupported && !search.VectorIndexSupported {
		fmt.Println("Note: --with-embeddings is ignored: this build has no vector-search support.")
		fmt.Println("      Rebuild with -tags vectors (FAISS) or -tags zvec to enable semantic search.")
